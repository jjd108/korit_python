...
#문법적으로는 특정 블록이 필요하지만, 실제로 수행할 작업이 없을 때 사용
# class car:
#     pass

class car:
    def __init__(self, model, price): # 생성자, 객채가 생성될 때 자동으로 실행
        # self는 현재 생성되는 객체 자신을 의미
        self.model = model
        self.price = price
        print(f"모델 이름:{self.model}, 가격: {self.price}객체 생성")

    def __del__(self): #소멸자, 객체가 메모리에서 삭제될 떄 자동으로 호출
        print(f"{self.model}의 객체가 소멸됨!")

    def drive(self, speed, distance):
        #seld는 이 메소드를 호출하는 객체가 자기 자신을 참조
...
#클래스를 통해 직접 매ㅔ소드를 호출, 이 경우 첫번쨰 인자로 객체 자신을 명시적으로 전달
