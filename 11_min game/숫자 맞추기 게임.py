...
difficulty_input = "" #입력받을 난이도 변수
max_try =0 #최대 시도 횟수
max_range = 0 #최대 숫자 범위

while True:
    difficulty_input = input("난이도를 선택해 주세요. (쉬움, 보통, 어려움) : ")
    if difficulty_input in difficulty:
        break
    else:
        print("난이도를 선택해 주세요. (쉬움, 보통, 어려움)")
        continue
...
print(f"숫자 맞추기 게임을 시작하겠습니다. \n난이도 : {difficulty}, 최대 시도 가능 횟수 - {difficulty}")
...
while True:
    ...
    if correct_answer > guess: # 정답이 입력한 숫자보다 큰 경우
        print("UP!")
elif correct_answer < guess: # 젇답이 입력한 숫자보다 작은 경우
        print("DOWN!")
else:
