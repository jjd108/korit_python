...
class Car(Vehicle):
    ...
    def display_info(self):
        #super를 사용하여 부모 클래스 display_info 매소드를 호출하고
...
motorcycle1 = motorcycle("Harley-dabidson", 2022, Cruiser )

#오토바이 클래스
#생성자는  brand, year, type(Cruiser)
#wheelie 매소드 => 브랜드 연식 오토바이가 윌리를 합니다! 출력
...
class Motorcycle(Vehicle):
    ...
    def Wheelie(self):
        print(f"브랜드: {self.brand}, 연식{self.year},Cruiser{self.type} ")
...
#class Manager
#department
#difplay_info => 부모꺼 호출해서 출력하고 departmenr따로 또 출력
...
motorcycle1.assign_task("새로운 프로젝트 착수")
